// Send the Appfi × ItemIQ Subcontractor Services Agreement to Aidan Dizaji.
...
// Prime Contractor (ItemIQ Inc.) countersignature.
//
// Run: node scripts/send-aidan.mjs
...
// Prime Contractor (ItemIQ) signature block on page 10.
...
const payload = {
  ...
  message:
    'Hi Aidan — please countersign the Subcontractor Services Agreement. Appfi has already signed the subcontractor side; you just need to fill out the Prime Contractor block on the last page (signature, name, title, email, date). Reach out if anything looks off.',
  documents: [
    {
...
console.log(`✓ Sent\n`)
console.log(`→ aidan.dizaji@gmail.com should have an email shortly.`)
